...
const Movie = () => {
  ...
  const movieData = movies.map((data) => (
    <Link to={`/showInfo/${data.id}`} key={data.id}>
      <div className="h-52 mb-5  md:mb-0 object-cover hover:transform hover:translate-x-4 hover:translate-y-2 transition-transform duration-300 relative">
        <img
  ...
  return (
    <div>
      ...
      <div className="container mx-auto">
        ...
        <div className="mx-10 md:mx-0 grid grid-cols-1 md:grid-cols-7 md:gap-2 lg:gap-4  bg-white md:shadow-lg overflow-hidden ">
            {movieData.slice(0,7)}
        </div>
